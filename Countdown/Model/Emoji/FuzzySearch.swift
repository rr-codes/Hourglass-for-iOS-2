...
extension Sequence where Iterator.Element == String {
    ...
    /**
     ...
     - parameter limit: The upper bound for the number of results to produce
     - returns: An ordered set of Strings based on whichever element matches closest to the `pattern` parameter.
     */
    public func sortedByFuzzyMatchPattern(_ pattern: String, loc: Int? = 0, distance: Double? = FuzzyMatchingOptionsDefaultValues.distance.rawValue, limit: Int? = nil) -> [String] {
        ...
        for element in 1..<10 {
            ...
            var options = FuzzyMatchOptions(threshold: Double(Double(element) / Double(10)), distance: FuzzyMatchingOptionsDefaultValues.distance.rawValue)
